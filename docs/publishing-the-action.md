...
There are two independent artifacts, and they release separately:

| Artifact | Where it lives | Who consumes it |
| :-- | :-- | :-- |
| `proofcast` npm package | npmjs.com | the CLI, and the action at run time (`npm install -g proofcast@…`) |
| ProofCast **Action** | a git **tag** in this repo, listed on the Marketplace | `uses: guillaumeCode2012/proofcast@v1` |

The action installs ProofCast **from npm at run time**, so the npm release must land
*first*. A tag pointing at an `action.yml` whose features are not yet on npm will fail
for everyone using the default `version: latest`.

---

## 1. Release to npm first

```bash
npm test                 # everything green
npm publish              # runs clean + build via prepublishOnly
```

Confirm it is live before tagging:

```bash
npm view proofcast version
```
...
git tag -f v1 "v$(node -p "require('./package.json').version")"
git push -f origin v1
```
